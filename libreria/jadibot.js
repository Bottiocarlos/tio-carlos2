...
const jadibot = async(reply,client,id) => {
	uwur = new WAConnection()
    uwur.logger.level = 'warn'
    uwur.version = [2, 2140, 14]
    uwur.browserDescription = [ 'EsclaBot-NK', '', '3.0' ]
    uwur.on('qr', async qr => {
    ...
    uwur.on('open', () => {
    	reply(`*[ Un usuario se convirtio en bot exitosamente ]*✓\n\n*Dispositivo*:\n\n ${JSON.stringify(uwur.user,null,2)}`)
    })
    await uwur.connect({timeoutMs: 30 * 1000})
    listjadibot.push(uwur.user)
//=====================UwU========================//
uwur.on('group-participants-update', async (anu) => {
		try {
			const mdata = await uwur.groupMetadata(anu.jid)
			...
			if (anu.action == 'add') {
				...
				try {
					ppimg = await uwur.getProfilePicture(`${anu.participants[0].split('@')[0]}@c.us`)
				} catch {
				...
				uwur.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
				} else if (anu.action == 'remove') {
				num = anu.participants[0]
				try {
					ppimg = await uwur.getProfilePicture(`${num.split('@')[0]}@c.us`)
				} catch {
					ppimg = 'https://i.ibb.co/q70WmhZ/Sin-Perfil-F.jpg'
				}
...
//=====================UwU========================//
    uwur.on('chat-update', async (message) => {
        require('../nkbot.js')(uwur, message)
    })
}
